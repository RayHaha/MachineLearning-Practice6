...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	while(again){
		...
		for(int i=0; i<1000; i++){
			...
			back[0] = (-1)*data[i][20];
			for(int j=0; j<20; j++){
				s = s + (-1)*data[i][20]*w[j+1]*data[i][j];
				back[j+1] = (-1)*data[i][20]*data[i][j];
			}
			double thita = 1/(1+exp((-1)*s));
			for(int j=0; j<21; j++){
				eingra[j] = eingra[j] + thita*back[j];
			}
		}
		for(int i=0; i<21; i++){
			eingra[i] = eingra[i]/1000;
		}
